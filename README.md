...
Создаем схему БД пользователя в файле schema.sql. Это позволит нам управлять аккаунтами
...
логином. Для входа под другим логином требуется перегрузить приложение.\
Достоинством работы со схемой БД получаемой из файла schema.sql является возможность управлять
аккаунтами пользователей из внешних источников, таких как data.sql. Недостатки подхода
с накладываемыми ограничениями на схему БД устранены. 
